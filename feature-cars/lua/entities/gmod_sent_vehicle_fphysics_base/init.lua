...
function ENT:PlayerSteerVehicle(ply, left, right)
	...
	local autoTurnsDisable = tobool(ply:GetInfo('cl_dbg_turnsignaloff'))
	...
	if autoTurnsDisable and oldAng > 5 and self.SmoothAng <= 5 and self.turnMode == 3
	or autoTurnsDisable and oldAng < -5 and self.SmoothAng >= -5 and self.turnMode == 2 then
		self:TurnSignal(0)
	end
...
function ENT:Lock()
	...
	if simfphys.HasAlarm(self) then simfphys.StopAlarm(self) end
end
...
function ENT:CanDrive( ply )
	if not ply:Alive() or ply:IsGhost() or ply:IsHandcuffed()
	or ply:getJobTable().notHuman then
		return false
	end

	local owner = self:CPPIGetOwner()
	if ply == owner or self.lockpicked then return true end

	local isFriend = IsValid(owner) and owner.Buddies and owner.Buddies[ply] and table.HasValue(owner.Buddies[ply], true)
	if isFriend or ply:Team() == TEAM_ADMIN then return true end
...
local function getEnterTime(ply)
	local wep = ply:GetActiveWeapon()

	if ply:GetNetVar('HandHold') or (wep.IsOctoWeapon and not wep.IsLightweight) then
		return 4
	end
...
function ENT:SetPassenger( ply, instant )
	if not IsValid(ply) or ply:IsGhost() then return end
	...
	ply.entering = true
	ply:EmitSound('octoteam/vehicles/shared/car_door_open_' .. math.random(1,2) .. '.ogg')
	if self.police then
		timer.Simple(0.5, function()
			ply:EmitSound('octoteam/vehicles/shared/car_open_door_alarm.ogg')
		end)
	end
	ply:DelayedAction('enterCar', L.enter_car, {
		time = instant and 0 or getEnterTime(ply),
		check = function()
		...
		succ = function()
			...
			ply.entering = nil
			ply:EmitSound('octoteam/vehicles/shared/car_door_close_' .. math.random(1,2) .. '.ogg')
			if self.police then ply:StopSound('octoteam/vehicles/shared/car_open_door_alarm.ogg') end
		end,
		fail = function()
			ply.entering = nil
			ply:EmitSound('octoteam/vehicles/shared/car_door_close_' .. math.random(1,2) .. '.ogg')
...
function ENT:OnRemove()
	...
	self:OnDelete()
	hook.Run('dbg-cars.onCarRemoved', self)
end
...
function ENT:TurnSignal(mode)
	...
	if IsValid(self:GetDriver()) and not tobool(self:GetDriver():GetInfo('cl_dbg_turnsignaloff')) then return end
