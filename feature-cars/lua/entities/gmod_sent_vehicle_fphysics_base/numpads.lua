...
netstream.Hook('dbg-cars.belt', function(ply)
	...
	if not ply:GetNetVar('belted') then
		...
		ply:EmitSound('octoteam/vehicles/shared/car_seatbelt_1.ogg')
		...
		ply:DoAnimation(ACT_SIGNAL_HALT)
		ply:EmitSound('octoteam/vehicles/shared/car_seatbelt_2.ogg')
...
local function getExitTime(ply)
...
hook.Add('CanExitVehicle', 'dbg-cars.unbelt', function(veh, ply)
	...
	if parent and parent.IsSimfphyscar then
		ply:EmitSound('octoteam/vehicles/shared/car_door_open_' .. math.random(1,2) .. '.ogg')
		if parent.police then
			timer.Simple(0.5, function()
				ply:EmitSound('octoteam/vehicles/shared/car_open_door_alarm.ogg')
			end)
		end
		ply:DelayedAction('vehicle-exit', 'Выход из машины', {
			time = getExitTime(ply),
			check = function()
				return IsValid(ply) and ply:Alive() and ply:GetVehicle() == veh
			end,
			succ = function()
				ply:ExitVehicle()
				ply:EmitSound('octoteam/vehicles/shared/car_door_close_' .. math.random(1,2) .. '.ogg')
				if parent.police then ply:StopSound('octoteam/vehicles/shared/car_open_door_alarm.ogg') end
			end,
		})
...
numpad.Register('k_lock', function( sid, ent, keydown )
	...
	if keydown then
		if ent:GetIsLocked() then
			ent:UnLock()
			ent:EmitSound("octoteam/vehicles/shared/car_door_unlock.ogg", 75, 100, 0.35)
		else
			...
			ent:Lock()
			ent:EmitSound("octoteam/vehicles/shared/car_door_lock.ogg", 75, 100, 0.35)
		end
