...
function PANEL:SetVehicle(class, data)
	...
	if t.id then
        n:AddLine("Сигнализация", (t.data.hasAlarm or e.hasAlarm) and "Да" or "Нет")
    else
        n:AddLine("Сигнализация", e.hasAlarm and "Да" or "Нет")
    end
    if t.data.hasTint then
        carDealer.tint.set(self.mdl.Entity, t.data.tintData)
    end
	self.mdlButs:Clear()
