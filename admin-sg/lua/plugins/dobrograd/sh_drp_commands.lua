...
command.permissions	= {"DBG: Использовать Set model на себе", "Set model"}
command.immunity 	= SERVERGUARD.IMMUNITY.LESSOREQUAL

function command:OnPlayerExecute(player, target, arguments)
	if not player:query("Set model") and player ~= target then return false end
...
function command:OnPlayerExecute(ply, target, arguments)
	...
	target:addMoney(math.max(target:GetNetVar("money")*-1, amount), 'dbgMoney.adminGive')
	hook.Run("octoinv.adminGiveMoney", ply, amount)
...
function command:OnPlayerExecute(ply, target, arguments)
	...
	hook.Run("octoinv.adminGiveKarma", ply, target, amount)
...
if SERVER then
	netvars.Register('notickets', {
		checkAccess = DarkRP.isAdmin,
	})
end

command.help = 'Запретить отправлять тикеты'
command.command = 'denytickets'
command.arguments = {'Никнейм или SteamID', 'Длительность', 'Причина'}
command.maxLength = '30d'
command.allowPerma = true
command.permissions = L.permissions_admin_commands
command.immunity = SERVERGUARD.IMMUNITY.LESSOREQUAL

function command:Execute(ply, _, args)
	local target = getSteamID(ply, args[1])
	if not target then return end

	local time = util.ParseDuration(args[2])
	if time < 0 then return end
	...
	local adminNotify = 'Ты установил запрет на отправление тикетов для %s на %s по причине: "%s"!'
	local plyNotify = 'Ты получил запрет на отправление тикетов администрации от %s на %s по причине: "%s"!'

	local denyData = {
		time = (time == 0 and true) or (os.time() + time),
		timeout = time,
		admin = IsValid(ply) and ply:SteamID() or 'консоли',
		reason = reason
	}

	octolib.setDBVar(target, 'notickets', denyData):Then(function()
		local targetPly = player.GetBySteamID(target)

		if IsValid(targetPly) then
			targetPly:SetNetVar('notickets', denyData.time == true or (CurTime() + (denyData.time - os.time())))
		end

		if IsValid(ply) then
			ply:Notify(adminNotify:format(
					IsValid(targetPly) and targetPly:Name() or target,
					octolib.time.formatDuration(time),
					reason
				)
			)
		else
			octolib.msg(adminNotify, target, time, reason)
		end
		...
		octolib.notify.send(
			...
			'warning',
			plyNotify:format(
				IsValid(ply) and ply:Name() or 'консоли',
				octolib.time.formatDuration(time),
				reason
			)
		)
...
command.help = 'Разрешить отправлять тикеты'
command.command = 'allowtickets'
...
function command:Execute(ply, _, args)
	...
	local notifyText = 'Запрет на отправку тикетов для %s снят'

	octolib.func.chain({
		function(nxt)
			octolib.getDBVar(target, 'notickets', nil):Then(nxt)
		end,
		function(nxt, data)
			if not data then return end
			octolib.setDBVar(target, 'notickets', nil):Then(nxt)
		end,
		function()
			local targetPly = player.GetBySteamID(target)

			if IsValid(targetPly) then
				targetPly:SetNetVar('notickets', nil)
			end

			if IsValid(ply) then
				ply:Notify(notifyText:format(target))
			else
				octolib.msg(notifyText, target)
			end
		end,
	})
...
command.arguments	= {"player", "length", "reason"}
command.maxLength	= '30d'
command.allowPerma	= true
command.permissions	= L.permissions_admin_commands
command.immunity 	= SERVERGUARD.IMMUNITY.LESSOREQUAL
...
function command:Execute(ply, _, arguments)
	...
	local adminNotify = 'Ты установил запрет на игру криминала для %s на %s по причине: "%s"!'
	local plyNotify = 'Ты получил запрет на игру криминала от %s на %s по причине: "%s"!'
...
function command:Execute(ply, _, arguments)
	...
	local adminNotify = 'Ты установил запрет на игру полиции для %s на %s по причине: "%s"!'
	local plyNotify = 'Ты получил запрет на игру полиции от %s на %s по причине: "%s"!'
