...
if (SERVER) then
	local notifyCommands = octolib.array.toKeys{ 'ban', 'unban', 'kick', 'rank', 'loocmute', 'oocmute', 'mute', 'gag', 'loocunmute', 'oocunmute', 'unmute', 'ungag', 'goomute', 'goounmute' }
	...
	local function RUN_COMMAND(commandTable, player, bIsSilent, arguments)
		...
		for k, argumentName in ipairs(commandTable.arguments or {}) do
			if not (argumentName == 'duration' or argumentName == 'length') then continue end

			-- Checking for exceed shortened time
			if string.find(arguments[k], '[wny]') and IsValid(player) and not player:IsSuperAdmin() then
				return serverguard.Notify(player, SGPF('exceed_short_duration'))
			end

			local maxLength = commandTable.maxLength
			if maxLength and IsValid(player) and not player:IsSuperAdmin() then
				local allowPerma = commandTable.allowPerma
				local length = util.ParseDuration(arguments[k])
				maxLength = util.ParseDuration(maxLength)

				if length == 0 and allowPerma == false then
					return serverguard.Notify(player, SGPF('exceed_max_length', octolib.time.formatDuration(maxLength * 60)))
				end
			...
			if (hook.Call("serverguard.PlayerCanUseCommand", nil, player, commandTable, bIsSilent, arguments) != false) then
				...
				if (commandTable.permissions and #commandTable.permissions > 0) then
					if (!serverguard.player:HasPermission(player, commandTable.permissions)) then
						serverguard.Notify(player, SERVERGUARD.NOTIFY.RED, string.format("У тебя недостаточно прав для этого, %s.", player:Nick()));
						return true;
					end;
				end;
				...
				elseif (!util.IsConsole(player) and #arguments == 0 and commandTable.arguments and #commandTable.arguments == 1 and commandTable.arguments[1] == "player") then
					...
					serverguard.Notify(player, SERVERGUARD.NOTIFY.RED, "Команда была введена неверно:");
