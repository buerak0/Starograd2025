...
-- Система Денежных переводов через телефон
-- function ply:TransferBalance(...)
--     if not self:HasMobilePhone() then return end
...
--     end
-- end
-- Сначала нам нужна функция для валидации суммы перевода
-- function ply:CanAffordTransfer(amount)
--     -- Проверяем, хватает ли у игрока денег на перевод
--     return self:getDarkRPVar("money") >= amount
-- end

-- -- Функция для перевода денег
-- -- принимает двух игроков (отправителя и получателя) и сумму перевода
-- function ply:TransferMoney(target, amount)
--     if not self:HasMobilePhone() then return end
--     if not self:CanAffordTransfer(amount) then return end
--     if not target:HasMobilePhone() then return end

--     -- Если все проверки прошли успешно, проводится перевод денег
--     self:addMoney(-amount)
--     target:addMoney(amount)

--     -- Добавляем уведомление о переводе
--     self:SendSMS("Вы перевели " .. tostring(amount) .. " игровой валюты игроку " .. target:Nick() .. ".")
--     target:SendSMS("Вы получили " .. tostring(amount) .. " игровой валюты от игрока " .. self:Nick() .. ".")
-- end
------------------------------------------------------
...
local function updateTypeStatus(ply, state, sounds)
	...
	if not ply:Alive() or ply:IsGhost() or not ply:HasMobilePhone() or ply:GetNetVar('onCall') or ply:GetNetVar('isCalling') then return end
