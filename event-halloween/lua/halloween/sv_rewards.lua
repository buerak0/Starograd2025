...
function halloween.sendItems(ply, toSend)
	...
	timer.Simple(math.random(60, 120), function()
		...
		ply:AddMarker({
			txt = 'Рождественская доставка',
			pos = box:GetPos() + Vector(0,0,40),
			col = Color(255,92,38),
			des = {'timedist', { 600, 100 }},
			icon = 'octoteam/icons-16/lorry.png',
		})
		ply:Notify('Твой рождественский подарок доставлен в почтовый ящик!')
	end)
	return true
...
function halloween.claimCase(ply, id, amount, func)
	...
	if ply:GetNetVar('sweets', 0) < cost then
		ply:Notify('warning', 'У тебя недостаточно ёлочных игрушек!')
		return func(false)
	end
...
netstream.Hook('dbg-halloween.flushRewards', function(ply, ent)
	...
	octochat.talkTo(ply, octochat.textColors.rp, 'Санта Клаус говорит: ', color_white, 'Перевел ' .. DarkRP.formatMoney(money) .. ' тебе на банковский счет')
	netstream.Start(ply, 'dbg-halloween.closeRewards')
end)
...
local function jackInteraction(self, name, _, ply)
	...
	if ply:GetNetVar('sweets', 0) <= 0 then
		...
		return octochat.talkTo(ply, octochat.textColors.rp, 'Санта Клаус говорит: ', color_white, 'Нет ёлочных игрушек? Очень жаль. подходи когда будут!')
	end
	...
	if mod > 0 then
		octochat.talkTo(ply, octochat.textColors.rp, 'Санта Клаус говорит: ', color_white, ('О, мне про тебя рассказывали! Сделаю-ка я тебе скидку в %s%%'):format(mod * 100))
	end
...
local jackSpawns = {
    ...
    rp_evocity_dbg_240912 = { Vector(-5153.3, -4368, 273), Angle(0, 90, 0) },
}
local function respawnJack()
	...
	jack:SetModel('models/Humans/Group01/male_08.mdl') -- Моедль
	jack:SetSkin(1)
