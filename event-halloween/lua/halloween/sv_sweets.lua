...
function plyMeta:AddSweets(sweets, msg)
	...
	if sweets > 0 then
		...
		self:Notify('hint', msg:format(sweets .. ' ' .. octolib.string.formatCount(sweets, 'ёлочную игрушку', 'ёлочные игрушки', 'ёлочная игрушка')))
	end
end
...
octochat.registerCommand('!sweets', {
	...
	execute = function(ply, _, args)
		...
		ply:Notify('rp', target:Name(), ' получает ', tostring(amount), ' ', octolib.string.formatCount(amount, 'ёлочную игрушку', 'ёлочные игрушки', 'ёлочная игрушка'))
