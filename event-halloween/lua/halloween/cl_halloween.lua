local msg = [[  Раздача подарков за "Ёлочные Игрушки"!

	В наш городок приехал славный Санта-Клаус, это такой весёлый дедок, который будет рад подарить тебе подарок и исполнять твоё желание. 

  	Поспеши 1 января он запрягает своих оленей и улетает!]]
...
local function paintPanel(_, w, h)
	...
	local mat = octolib.getImgurMaterial('Cs4KKEN.png')
...
hook.Add('octogui.f4-tabs', 'halloween', function()
	...
	octogui.addToF4({
		order = 0.1,
		id = 'halloween',
		name = 'Рождественский переполох',
		icon = Material('octoteam/icons/xmas_ball.png'),
		build = function(f)
			f:SetSize(500, 500)
...
hook.Add('InitPostEntity', 'dbg-halloween', function()
	if game.GetMap() == 'rp_evocity_starograd_250823' then
