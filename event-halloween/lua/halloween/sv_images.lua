	...
	function()
		return 'Получить', function(ply, data)
			if not ply.halloweenTheme then
				return ply:Notify('warning', 'А как же рождественское настроение? Активируй его во вкладке "Рожденство" в F4-меню!')
			end
...
hook.Add('dbg-char.firstSpawn', 'dbg-halloween.images', function(ply)
	if game.GetMap() ~= 'rp_evocity_starograd_250823' then return end
...
hook.Add('octolib.event:halloween.done', 'dbg-halloween.images', function(data)
	octolib.notify.send(util.SteamIDFrom64(data.user), 'addSweets', 'Тебе прислали ёлочные игрушки. Забери их в F4', {
		text = ('Тебе прислали %s %s'):format(reward, octolib.string.formatCount(reward, 'ёлочную игрушку', 'ёлочные игрушки', 'ёлочная игрушка')),
		v = reward,
	})
end)

hook.Add('PlayerCanOOC', 'dbg-halloween.images', function(_, txt)
	if txt:find('sas.ru', 1, true) then
