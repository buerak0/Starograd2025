...
local minTimeToBuild = 1 * 1 * 1 -- 1 hour
...
local function canSpawn(ply)
	...
	if ply:TriggerCooldown('lookbook', 10) then
		local request = {{
			name = 'Стиль построек Старограда',
			...
			buttonText = 'Лукбук Старограда',
			buttonURL = 'https://discord.gg/v2YDDYkDpy',
		}}
		if CFG.dev or ply:GetTimeTotal() >= minTimeToBuild then
			request[#request + 1] = {
				desc = 'Если администраторы посчитают твою постройку нарушающей общую стилистику Староград, они в уведомлении попросят тебя исправить что-то в ней или перестроить вовсе. Если эти просьбы будут проигнорированы, им придется удалить постройку.\n\nОтметь ниже, когда ознакомишься с лукбуком, осознаешь важность поддержания общей стилистики Старограда и будешь готов украшать город своей постройкой:',
				type = 'check',
				txt = 'Вперед играть и строить!!',
				required = true,
			}
		else
...
local timeForAD2 = 3 * 60 * 60 -- 3 hours
