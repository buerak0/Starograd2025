...
local function sortCars(cars)
	local sorted = {}

	for id, car in pairs(cars) do
		local cat = car.category
...
concommand.Add('octoinv_editor', function()
	...
	function l:OnRowRightClick(id, line)
		...
		m:AddOption("Управление гаражом", function() netstream.Start('octoinv.gareditor.open', line.steamID, line:GetColumnText(2)) end):SetIcon('icon16/car.png')
...
--
-- Продам гараж: +79302211122
--

netstream.Hook('octoinv.gareditor.open', function(data)
	if IsValid(editorWindows[data.steamID]) then editorWindows[data.steamID]:Remove() end
	...
	w:SetWide(400)
	w:SetTall(data.noEdit and 250 or 300)
	w:AlignBottom(20)
	w:AlignLeft(20)
	w:SetTitle(data.steamID .. L.steamid_edit)
	w:MakePopup()
	...
	steamworks.RequestPlayerInfo(util.SteamIDTo64(data.steamID), function(n)
		if IsValid(w) then
			pl_name = n
			w:SetTitle(data.steamID .. ' (' .. n .. ')' .. " - Управление гаражом")
		end
	end)

	local Carlist = vgui.Create("DScrollPanel", w)
    Carlist:Dock(FILL)
	

	function listCars()

		if Carlist then Carlist:Clear() end

		local buttonContainer = vgui.Create("DPanel", Carlist)
    	buttonContainer:SetWide(400)

    	local btncount = 0

		for id, veh in pairs(data.cars) do
			btncount = btncount + 1
	        local btnDelCar = vgui.Create("DButton", buttonContainer)
	        btnDelCar:SetText(carDealer.vehicles[veh.class].name .. ' (' .. veh.plate .. ')', w)
	        btnDelCar:Dock( TOP )
			btnDelCar:DockMargin( 0, 0, 0, 5 )

	        btnDelCar.DoClick = function()
				local m = DermaMenu()

				m:AddOption("Изменить номерной знак", function()

					netstream.Start('octoinv.gareditor.editplate', pl_name, data.steamID, veh.plate)

					timer.Simple(0.5, function()
			        		netstream.Start('octoinv.gareditor.updategarage', data.steamID)
			        end)
					

				end):SetIcon(octolib.icons.silk16('track_changes_edit')) -- Знак!!!

				m:AddOption("Удалить", function() 
					octolib.confirmDialog(w, 'Вы действительно хотите удалить ' .. carDealer.vehicles[veh.class].name .. ' у игрока ' .. pl_name, function(ans) -- СЕЙВ2!!!
					if ans then
						netstream.Start('octoinv.gareditor.delcar', pl_name, data.steamID, veh.plate)
				...
				end):SetIcon('icon16/car_delete.png') -- Удаление!!!
				m:Open()
	        end

		end

		netstream.Hook('octoinv.gareditor.updategarage.response', function(garage)
			data.cars = garage
		    listCars()
		end)

		buttonContainer:SetTall(btncount * 27)

		local variants = {
		"Ну же, всунь свою машину",
		"Ну что ты ждешь?! Засунь машину",
		"Ну давай уже, засунь машину",
		"Пустовато...",
		"Кажется тут пусто...",
		}

		if next(data.cars) == nil then
			local lbl_p = vgui.Create("DLabel", w)
			lbl_p:SetText(variants[math.random(1, #variants)])
			lbl_p:SizeToContents()
			local labelWidth = lbl_p:GetWide()
			local frameWidth = w:GetWide()

			lbl_p:SetPos((frameWidth - labelWidth) / 2, 150)
		end
	end
	...
	btmPan:SetPaintBackground(false)
 	...
 	lbl = octolib.label(w, '                                          Список авто пользователя') -- Сделай норм че за хуйня
 	
    local wide = math.max(400, surface.GetTextSize(lbl:GetText()) + 15)
	...
	local addCar = octolib.button(btmPan, 'Добавить', function() -- Добавление!!!
		local m = DermaMenu()

		local sortedCars = sortCars(carDealer.vehicles)

		for cat, vehs in pairs(sortedCars) do
			for id, veh in pairs(vehs) do
				local icon = cat_icon[veh.category]
				if not icon then icon = 'car2' end

		        m:AddOption('[' .. id .. '] ' .. veh.name, function() 

		        	netstream.Start('octoinv.gareditor.addcar', pl_name, data.steamID, id)

		        	timer.Simple(0.5, function()
			        		netstream.Start('octoinv.gareditor.updategarage', data.steamID)
			        end)
		        end):SetIcon(octolib.icons.color(icon))
	...
	function w:OnRemove()
		netstream.Start('octoinv.gareditor.close', data.steamID)
	end
...
netstream.Hook('octoinv.gareditor.change', function(steamID)
	if IsValid(editorWindows[steamID]) then
		editorWindows[steamID].saved = false
	else
		netstream.Start('octoinv.gareditor.close', steamID)
	end
end)
