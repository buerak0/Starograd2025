...
netstream.Hook('octoinv.gareditor.open', function(ply, steamID, name)
	...
	octolib.func.chain({
		...
		function(done)
			carDealer.getGarage(steamID, function(garage)
				done(garage)
			end)
		end,
		function(done, garage)
			...
			local function addCont(contID, contData)
				...
				cont:Hook('canMoveOut', 'octoinv.gareditor', editorInteractHook)
				cont:Hook('canMoveIn', 'octoinv.gareditor', editorInteractHook)
				cont:Hook('canSeeUseList', 'octoinv.gareditor', editorInteractHook)
				cont:Hook('canUse', 'octoinv.gareditor', editorInteractHook)
				cont:Hook('canDrop', 'octoinv.gareditor', editorInteractHook)
			...
			netstream.Start(ply, 'octoinv.gareditor.open', {
				steamID = steamID,
				cars = garage,
...
netstream.Hook('octoinv.gareditor.addcar', function(ply, target, steamID, veh)
	if not isAllowed(ply) then
		ply:Notify('warning', L.not_have_access)
		return
	end

	carDealer.ownVeh(steamID, veh, function(id, plate)
		carDealer.notify(ply, 'Игроку ' .. target .. ' в гараж доставлен ' .. 'Автомобиль' .. ' с регистрационным номером ' .. plate)
	end)
end)

netstream.Hook('octoinv.gareditor.updategarage', function(ply, steamID)
	if not isAllowed(ply) then
		ply:Notify('warning', L.not_have_access)
		return
	end
	...
	carDealer.getGarage(steamID, function(garage)
		netstream.Start(ply, 'octoinv.gareditor.updategarage.response', garage)
	end)
	
end)


netstream.Hook('octoinv.gareditor.editplate', function(ply, target, steamID, plate_past)
	if not isAllowed(ply) then
		ply:Notify('warning', L.not_have_access)
		return
	end
	...
	octolib.request.send(ply, {{
		...
		plate = {
			...
			carDealer.getVehByPlate(plate_past, function(f_veh)
				local vehID = f_veh.id				
				...
				carDealer.getVehByPlate(plate, function(veh)
					...
					carDealer.updateVehData(vehID, { plate = plate }, function()
						hook.Run('car-dealer.plateChanged', vehID, steamID, plate)
					end)
					...
					carDealer.notify(ply, 'У игрока ' .. target .. ' изменен номер на ' .. f_veh.class .. ' с ' .. plate_past .. ' на ' .. plate)
			end)
		end)
	end)
end)

netstream.Hook('octoinv.gareditor.delcar', function(ply, target, steamID, plate)
	if not isAllowed(ply) then
		ply:Notify('warning', L.not_have_access)
		return
	end
	...
	carDealer.getVehByPlate(plate, function(veh)
		carDealer.unownVeh(veh.id, function(veh)
			carDealer.notify(ply, 'У игрока ' .. target .. ' удален ' .. 'Автомобиль' .. ' с регистрационным номером ' .. plate)
		end)
	end)

	--ply:notify(ply, 'У игрока ' .. target .. ' из гаража удален ' .. veh .. ' с регистрационным номером ' .. plate)
end)
...
netstream.Hook('octoinv.gareditor.close', function(ply, steamID)
