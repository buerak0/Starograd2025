...
local applyCooldown = 2 * 60 -- 10 minutes
...
inCopLimit = octolib.array.toKeys {'medcop', 'cop', 'cop2', 'chief', 'dpd'}
avoidLimit = octolib.array.toKeys {'chief', 'mayor'}
function inPoliceLimits()
...
net.Receive('dbg-police.apply', function(len, ply)
	...
	--if not inPoliceLimits() then   -- ОГРАНИЧЕНИЕ ПОЛИЦИИ МУНИКОВ!!!!!!!!!!
