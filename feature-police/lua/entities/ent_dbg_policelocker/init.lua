...
local mdls = {
	male = {
		{ 'models/player/octo_sheriff/male_01.mdl' },
		{ 'models/player/octo_sheriff/male_02.mdl' },
		{ 'models/player/octo_sheriff/male_03.mdl' },
		{ 'models/player/octo_sheriff/male_04.mdl' },
		{ 'models/player/octo_sheriff/male_05.mdl' },
		{ 'models/player/octo_sheriff/male_06.mdl' },
		{ 'models/player/octo_sheriff/male_07.mdl' },
		{ 'models/player/octo_sheriff/male_08.mdl' },
		{ 'models/player/octo_sheriff/male_09.mdl' },
	},
	female = {
	...
	if equip then
		if job.police then
			...
			if pJobID ~= TEAM_MAYOR then
				...
				ply:SetBodygroup(4, hat and (cmd == 'chief' and 0 or 1) or 1)
			end  
            ply:SetClothes(nil)
			ply:changeTeam(pJobID, true, true)
			if pJobID ~= TEAM_MAYOR then
				...
				local cmd = pJob.command
				ply:SetSkin(skins[cmd])
				ply:SetBodygroup(11, hat and (cmd == 'chief' and 3 or 1) or 1)
			end   
            ply:SetClothes(nil)
			ply:changeTeam(pJobID, true, true)
			if pJobID ~= TEAM_MAYOR then
				...
				local cmd = pJob.command
				ply:SetSkin(skins[cmd])
				ply:SetBodygroup(13, hat and (cmd == 'chief' and 1 or 1) or 1)
			end   
            ply:SetClothes(nil)
			ply:changeTeam(pJobID, true, true)
			if pJobID ~= TEAM_MAYOR then
				...
				local cmd = pJob.command
				ply:SetSkin(skins[cmd])
				ply:SetBodygroup(3, hat and (cmd == 'chief' and 0 or 2) or 2)
			end     
			local medkit = ply:GetWeapon('med_kit')
			if IsValid(medkit) then medkit:SetClip1(200) end
			ply:SetAmmo(180, 'pistol')
