	...
	if equip then
		...
		function m:UpdateData()
			...
			if job ~= 'mayor' then
				...
				self.Entity:SetBodygroup(3, p.hat and (job == 'chief' and 3 or 0) or 0)
			else
...
--function h:OnChange(val)
		--	if val then
		--		p.hat = false
			--	m.Entity:SetBodygroup(3, job == 'chief' and 1 or 1)
		--	else
			--	p.hat = true
		--		m.Entity:SetBodygroup(3, 1)
		--	end
		--end   
       -- local h = p2:Add 'DCheckBoxLabel'
		--h:Dock(TOP)
		--h:DockMargin(0,5,0,5)
		--h:SetText(L.beltforpolice)
		--h:SetValue(2)
		--function h:OnChange(val)
		--	if val then
		--		p.hat = true
		--	m.Entity:SetBodygroup(3, job == 'chief' and 3 or 3)
		--	else
		--		p.hat = false
			--	m.Entity:SetBodygroup(3, 0)
		--	end
		--end   
