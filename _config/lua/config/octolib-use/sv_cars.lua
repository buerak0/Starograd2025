...
CFG.use.gmod_sent_vehicle_fphysics_base = {
	...
	function(ply, ent)
		...
		if ply:Team() == TEAM_DPD then
			...
			if mdl:find('octo_dpd/') then
				bodygroup, armorOn, armorOff = 10, 50, 28
    ...
    function(ply, ent)
		...
		if ply:Team() == TEAM_DPD then
			local mdl = ply:GetModel()
			if mdl:find('octo_dpd/') then
				bodygroup, armorOn, armorOff = 7, 228, 28
			elseif mdl:find('dpdsl/') or mdl:find('dpdfem/') then
				bodygroup, armorOn, armorOff = 3, 50, 28
			else return end
		elseif ply:Team() == TEAM_DPD then
			bodygroup, armorOn, armorOff = 6, 50, 28
		elseif ply:Team() == TEAM_DPD then
			bodygroup, armorOn, armorOff = 6, 55, 45
		else return end

		takeOff = ply:GetBodygroup(bodygroup) == 1
		return (takeOff and 'Снять' or 'Надеть') .. ' шлем', 'octoteam/icons/helmet.png', function(ply, ent, args)
			local back = ent:NearestPoint(ent:WorldSpaceCenter() + ent.Forward * -1000)
			if (ply:GetShootPos() - back):Length2DSqr() > CFG.useDistSqr then
				return ply:Notify('warning', 'Подойди ближе к задней части автомобиля')
			end

			ply:DelayedAction('armor', (takeOff and 'Снятие' or 'Экипировка') .. ' шлема', {
				time = 5,
				check = function()
					return octolib.use.check(ply, ent)
				end,
				succ = function()
					ply:DoEmote('{name}, открыв багажник с помощью ключа, ' .. (takeOff and 'снимает' or 'надевает') .. ' презерватив на голову')
					ply:SetArmor(takeOff and armorOff or armorOn)
					ply:SetBodygroup(bodygroup, takeOff and 0 or 7)
				end,
			}, {
				time = 1.5,
				inst = true,
...
function(ply, ent)
	...
	return 'Аммуниция', 'octoteam/icons/gun_rifle.png', function(ply, ent, args)
		...
		local opts = {
            {'AR-15', false, 'weapon_octo_ar15'},    
            {'REMIGTON', false, 'weapon_octo_remigton'},    
		...
		if cdData and cdData.glauncher and (ply:GetActiveRank('dpd') == 'swat' or ply:GetActiveRank('wcso') == 'seb') then
			...
			opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_m4a1'] and 'Вернуть ' or 'Взять ') .. 'M4A1',
				true, 'weapon_octo_m4a1'
			}
            opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_hk416'] and 'Вернуть ' or 'Взять ') .. 'HK416',
				true, 'weapon_octo_hk416' 
            }   
            opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_g36'] and 'Вернуть ' or 'Взять ') .. 'HK36',
				true, 'weapon_octo_g36' 
            }  
            opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_ar15'] and 'Вернуть ' or 'Взять ') .. 'AR-15',
				true, 'weapon_octo_ar15' 
            }                
            opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_sqdcl'] and 'Вернуть ' or 'Взять ') .. 'Снайперка с глушителем',
				true, 'weapon_octo_sqdcl' 
            }       
            opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_remigton'] and 'Вернуть ' or 'Взять ') .. 'REMIGTON',
				true, 'weapon_octo_remigton' 
            }               
			opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_m3'] and 'Вернуть ' or 'Взять ') .. 'M3',
				true, 'weapon_octo_m3'
			}
			opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_beanbag'] and 'Вернуть ' or 'Взять ') .. 'BeanBag',
				true, 'weapon_octo_beanbag'
			}
			opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_awp'] and 'Вернуть ' or 'Взять ') .. 'AWP',
				true, 'weapon_octo_awp'
			}
			opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_xm1014'] and 'Вернуть ' or 'Взять ') .. 'XM1014',
				true, 'weapon_octo_xm1014'
			}
			opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_g3sg1'] and 'Вернуть ' or 'Взять ') .. 'G3SG1',
				true, 'weapon_octo_g3sg1'
			}
			opts[#opts + 1] = {
				(ent.locker and ent.locker['weapon_octo_p90'] and 'Вернуть ' or 'Взять ') .. 'P90',
				true, 'weapon_octo_p90'
			}
			opts[#opts + 1] = {
				(ent.locker and ent.locker['dbg_shield'] and 'Вернуть ' or 'Взять ') .. 'щит',
				true, 'dbg_shield'
			}
		end
