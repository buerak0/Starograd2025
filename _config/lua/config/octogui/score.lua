...
local function paintButton(self, w, h)
	...
	local col = Color(11,43,75, 180 + al * 70)
	...
	--draw.SimpleText(team.GetName(ply:Team()), 'dbg-score.normal', w - 30, h/2 - 1, Color(238,238,238), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
...
p.Think = function( self )
	...
	if LocalPlayer():IsAdmin() and not IsValid(self.adminBut1) then
		...
		b1:AddHint('Режим Админа (ПКМ для больших возможностей)')
		function b1:DoClick()
				RunConsoleCommand('dbg_admin')
			b1.DoRightClick = function()
			local submenu = DermaMenu()
                    submenu:AddOption("Панель Игрового мастера", function()
                		RunConsoleCommand('gmpanel')
                end)
                	submenu:AddOption("Выдача Предметов", function()
                		RunConsoleCommand('octoinv_give')
                end)
                	submenu:AddOption("Скринграб", function()
                		RunConsoleCommand('gts')
				end)
                    submenu:AddOption("Посмотреть выброшенные вещи", function()
                		RunConsoleCommand('octoinv_stats')
				end)
                    submenu:AddOption("Редактор игроков", function()
                		RunConsoleCommand('octoinv_editor')
				end)
                   submenu:AddOption("Управление WhiteList`ом", function()
              		RunConsoleCommand('dbg_whitelist')
			end) 
				local mouseX, mouseY = gui.MousePos()
				submenu:Open(mouseX, mouseY)
			end
		end
...
hook.Add( 'RenderScreenspaceEffects', 'dbg-scoreboard', function()
	...
	if a > 0 then
		...
		draw.NoTexture()
		local col = colors.bg
		surface.SetDrawColor(11,43,75, a * 15)
