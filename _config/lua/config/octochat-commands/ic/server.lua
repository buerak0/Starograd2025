...
--             target:TransferBalance(octochat.textColors.rp, ply:Name(), L.owner_sms, Color(250,250,200), txt)
--         else
--             target:Notify('Тебе пришло SMS от ' .. ply:Name() .. ', но ты не можешь его прочитать, так как руки связаны')
--         end
--         ply:TransferBalance(octochat.textColors.rp, target:Name(), L.target_sms, Color(250,250,200), txt)
--         function(done)
...
--         finish()
--         BraxBank.UpdateMoney(id, balance + 10000)
...
octochat.registerCommand('/transfer', {
    ...
    execute = function(ply, _, args)
        ...
        local target, txt = octochat.pickOutTarget(args)
        if not target then
            return txt or 'Формат: /transfer "Имя игрока" сумма'
        end

        if txt == '' then
            return 'Формат: /transfer "Имя игрока" сумма'
        end
        ...
        if not target:IsHandcuffed() then
            if BraxBank.PlayerMoney(ply) >= tonumber(txt) then
                ply:DoEmote('{name} делает перевод.')
                BraxBank.UpdateMoney(ply, (BraxBank.PlayerMoney(ply)-tonumber(txt)), False)
                BraxBank.UpdateMoney(target, BraxBank.PlayerMoney(target)+tonumber(txt), False)
                if target:HasPhone() then 
                	target:SendSMS(octochat.textColors.rp, ply:Name(), ' перевел тебе: ', Color(250, 250, 200), txt .. "P")
                end
                -- local str_text = ply:Name(), ' перевел тебе: ', txt .. "$"
   				-- m_sendsmsm_tr(ply, target, txt)
              
            else
                return 'На счете не достаточно средств'
            end
        else
            target:Notify('Тебе пришел перевод от ' .. ply:Name() .. ', но ты не можешь его принять, так как находишься в наручниках.')
        end
    end
})
-- LOTTERY
