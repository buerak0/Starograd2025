...
octochat.registerCommand('/firemode', {
    cooldown = 1.5,
    execute = function(ply)
        ...
        ply:DoEmote('{name} переключает режим стрельбы')
        timer.Simple(1, function()

            if not IsValid(wep) then return 'Для этого нужно держать в руках оружие' end

            if wep.MultiMode and ply:KeyDown(IN_USE) then
                if wep:GetFireMode() == 'semi' then
                    wep:SetFireMode('burst')
                    wep.Primary.Automatic = false
                    ply:Notify(L.switched_burst)
                elseif wep:GetFireMode() == 'burst' then
                    wep:SetFireMode('auto')
                    wep.Primary.Automatic = true
                    ply:Notify(L.switched_fully_auto)
                elseif wep:GetFireMode() == 'auto' then
                    wep:SetFireMode('semi')
                    wep.Primary.Automatic = false
                    ply:Notify(L.switched_semi_auto)
                end
                wep:SetNextPrimaryFire(CurTime() + 0.5)
                wep:SetNextSecondaryFire(CurTime() + 0.5)
            else
                ply:Notify('Этому оружию нельзя сменить режим стрельбы')
            end
...
-- 	phone = true,
-- 	execute = function(ply)
-- 		ply:DoEmote('{name} отправляет деньги через телефон')
-- 		timer.Simple(1, function()
-- 			ply:Notify('Твой баланс в банке: ' .. DarkRP.formatMoney(BraxBank.PlayerMoney(ply)))
-- 		end)
-- 	end,
-- 	aliases = {'/transfer'},
-- })
-- где ply - отправитель, target - адресат, amount - сумма для перевода
...
local function ChangeBodyGroup(player, group_id, new_value)
    local bodyGroups = player:GetBodyGroups()
    local foundGroup = false
    print(group_id)
    print(new_value)
    for _, bodyGroup in pairs(bodyGroups) do
        if bodyGroup.id == group_id then
            player:SetBodygroup(group_id, new_value)
            foundGroup = true
            break
        end
    end
...
local function hasGasMask(player)
    if IsValid(player) and player:IsPlayer() then
        local bodyGroup_name = player:GetBodygroupName(6)
        local bodyGroups = player:GetBodyGroups()
        for _, bodyGroup in pairs(bodyGroups) do
            if bodyGroup.name == bodyGroup_name then
                return player:GetBodygroup(bodyGroup.id) == 2
            end
        end
...
octochat.registerCommand('/gasmask', {
    cooldown = 1.5,
    execute = function(ply)
        if not hasGasMask(ply) then   
            ply:TalkToRange(350, octochat.textColors.rp, ("%s надевает противогаз."):format(ply:Name()))
           	wearMask(ply, 'gasmask') 
        else
            ply:TalkToRange(350, octochat.textColors.rp, ("%s снимает противогаз."):format(ply:Name()))
        end
    end,
    check = function(ply)
