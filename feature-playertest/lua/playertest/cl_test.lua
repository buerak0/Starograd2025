...
local servers = {
    default = {
		name = 'Староград',
		desc = [[С ходом времени Староград стал расширяться. Староград – современный район города, расположившийся на обширной территории. Здесь есть районный центр, озеро с загородными коттеджами, а также промышленная зона
...
Благодаря просторной карте автомобильная инфраструктура развита, а слоты для игроков увеличены]],
	...
	['45.11.16.76:7028'] = {
		name = 'Белогорск',
		desc = [[Прошлый сервер от нашей команды (закрыт)]],
	},
}
...
function test.welcomeScreen(attempts, hint, showMsg)
	...
	local links = octolib.button(bp, 'Ссылки', function()
		local m = DermaMenu()
		m:AddOption(L.rules_server, function() octoesc.OpenURL('https://docs.google.com/document/d/1G12Rm-QVNtpbNHOE7_5rQWWLPdOzOKiO_xWYZjEfOh8/edit?tab=t.0') end)
		m:AddOption(L.forum, function() octoesc.OpenURL('https://discord.gg/v2YDDYkDpy') end)
		m:AddOption(L.wiki, function() octoesc.OpenURL('https://discord.gg/v2YDDYkDpy') end)
		m:AddOption(L.we_in_steam, function() octoesc.OpenURL('https://discord.gg/v2YDDYkDpy') end)
		m:AddOption(L.we_in_vk, function() octoesc.OpenURL('https://discord.gg/v2YDDYkDpy') end)
		m:AddOption(L.our_site, function() octoesc.OpenURL('https://discord.gg/v2YDDYkDpy') end)
		m:AddOption(L.write_us_in_vk, function() octoesc.OpenURL('https://discord.gg/v2YDDYkDpy') end)
		m:Open()
	end)
	links:SetWide(115)
	links:Dock(LEFT)
...
function test.create(quiz, toPass, onCancel, onSubmit)
	...
	local butRules = octolib.button(bp1, 'Правила', function()
		octoesc.OpenURL('https://docs.google.com/document/d/1G12Rm-QVNtpbNHOE7_5rQWWLPdOzOKiO_xWYZjEfOh8/edit?tab=t.0#heading=h.muqro4odkwta')
	end)
	butRules:Dock(LEFT)
...
local config = {
    ...
    rp_evocity_dbg_240112 = {
		...
		{
			start = {Vector(714, -5588, 538), Angle(10, 101, 0), 80},
			finish = {Vector(-887, -5050, 460), Angle(14, 59, 0), 80},
			time = 20,
		},
		{
