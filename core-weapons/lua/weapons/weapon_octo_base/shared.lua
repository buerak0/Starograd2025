...
function SWEP:CanFire()
	...
	if not self:GetNetVar('CanSetReady') or not self:GetNetVar('IsReady') or
		...
		or self.Owner:WaterLevel() == 3 or not self:GetNetVar('CanFire')
...
function SWEP:PrimaryAttack()
	...
	if not IsFirstTimePredicted() or not self:CanFire() or
		(self.nextFire or 0) > ct or self:GetNextPrimaryFire() > ct or not self.FireModes[self:GetNetVar('firemode')]
	then return end
