...
netstream.Hook('octoweapons-setReady', function(ply, b)
	local wep = ply:GetActiveWeapon()
	if not IsValid(wep) or not wep:GetClass():find('octo', 1, false) then return end

	wep:SetNetVar('CanSetReady', b)
end)
...
function SWEP:Initialize()
	...
	local firemode = self:GetNetVar('firemode')
...
function SWEP:Think()
	...
	if curAmmo[1] ~= self:Clip1() or curAmmo[2] ~= self:Ammo1() then self:SetNetVar('updateLook', true) end

	local ply = self.Owner
	if (not self:GetNetVar('dbgLook') and self.canLook) or self:GetNetVar('updateLook') then
		...
		self:SetNetVar('updateLook', false)
	end
	...
	if self:GetNetVar('IsReady') then
		if self.canLook then self:SetNetVar('dbgLook', nil) end
...
function SWEP:ChangeFireMode(mode)
	if not self.FireModes[tonumber(mode)] then return end

	self:SetNetVar('firemode', tonumber(mode))

	local strMode = self.FireModes[tonumber(mode)]
...
function SWEP:Reload()
	...
	self:SetNetVar('isReloading', true)
	...
	timer.Simple(self.ReloadTime + .3, function()
		if IsValid(self) then
			...
			self:SetNetVar('CanSetReady', true)
			self:SetNetVar('isReloading', false)
		end
	end)

	self:SetNetVar('updateLook', true)
end
